...
namespace ELODIE.Services.MAppUser
{
    ...
    public class AppUserValidator : IAppUserValidator
    {
        ...
        private async Task<bool> ValidateOrganization(AppUser AppUser)
        {
            OrganizationFilter OrganizationFilter = new OrganizationFilter
            {
                Id = new IdFilter { Equal = AppUser.OrganizationId }
            };

            int count = await UOW.OrganizationRepository.Count(OrganizationFilter);
            if (count == 0)
                AppUser.AddError(nameof(AppUserValidator), nameof(AppUser.Organization), ErrorCode.OrganizationNotExisted);
            return AppUser.IsValidated;
        }
        ...
        public async Task<bool> Delete(AppUser AppUser)
        {
            ...
            if (AppUser.Used)
                AppUser.AddError(nameof(AppUserValidator), nameof(AppUser.Id), ErrorCode.AppUserInUsed);
            return AppUser.IsValidated;
        }
        ...
        public async Task<bool> Import(List<AppUser> AppUsers)
        {
            ...
            foreach (var AppUser in AppUsers)
            {
                ...
                if (!SexIds.Contains(AppUser.SexId))
                {
                    AppUser.AddError(nameof(AppUserValidator), nameof(AppUser.Sex), ErrorCode.SexEmpty);
                }
                if (!OrganizationIds.Contains(AppUser.OrganizationId))
                {
                    AppUser.AddError(nameof(AppUserValidator), nameof(AppUser.Organization), ErrorCode.OrganizationNotExisted);
                }
            }
        ...
        public async Task<bool> AdminChangePassword(AppUser AppUser)
        {
            await ValidateId(AppUser);
            return AppUser.IsValidated;
        }
